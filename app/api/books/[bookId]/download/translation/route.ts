...
/**
 * Download book with translation only
 */
export async function GET(
  ...
  try {
    ...
    const { data: paragraphs, error: parasError } = await supabase
      ...
      .select('para_idx, text_translated')
      .eq('book_id', bookId)
    ...
    // Generate translation-only text
    let content = `${book.title} - 译文\n`
    if (book.author) {
    ...
    return new NextResponse(content, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="translation-${book.title}.txt"`,
      },
    })
  } catch (error: any) {
    console.error('[Download Translation] Error:', error)
    return NextResponse.json(
      { error: error.message || 'Failed to generate translation file' },
      { status: 500 }
    )
  }
}
